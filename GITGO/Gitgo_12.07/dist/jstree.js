...
(function (factory) {
  ...
  $.jstree.core.prototype = {
    ...
    init: function (el, options) {
      ...
      this.element.html(
        "<" +
          "ul class='jstree-container-ul jstree-container-ul1 jstree-children' role='group'><" +
          "li id='j" +
          this._id +
    ...
    get_node: function (obj, as_dom) {
      ...
      try {
        ...
        if (as_dom) {
          obj =
            obj.id === $.jstree.root ? this.element : $("#" + obj.id.replace($.jstree.idregex, "\\$&"), this.element);
        }
        return obj;
      } catch (ex) {
        return false;
    ...
    _load_nodes: function (nodes, callback, is_callback, force_reload) {
      ...
      for (i = 0, j = nodes.length; i < j; i++) {
        if (
          m[nodes[i]] &&
          ((!m[nodes[i]].state.loaded && !m[nodes[i]].state.failed) || (!is_callback && force_reload))
        ) {
              ...
              if (!m[dat[i].parent.toString()]) {
                if (typeof inst !== "undefined") {
                  inst._data.core.last_error = {
                    ...
                    data: JSON.stringify({ id: dat[i].id.toString(), parent: dat[i].parent.toString() }),
                  };
        ...
        try {
          if (this._wrk === null) {
            this._wrk = window.URL.createObjectURL(
              new window.Blob(["self.onmessage = " + func.toString()], { type: "text/javascript" })
            );
          }
        ...
        if (!is_callback) {
          par =
            obj.parent !== $.jstree.root
              ? $("#" + obj.parent.replace($.jstree.idregex, "\\$&"), this.element)[0]
              : null;
    ...
    check: function (chk, obj, par, pos, more) {
      ...
      if (
        chc === false ||
        ($.isFunction(chc) && chc.call(this, chk, obj, par, pos, more) === false) ||
        (chc && chc[chk] === false)
      ) {
        this._data.core.last_error = {
          error: "check",
    ...
    edit: function (obj, default_text, callback) {
      ...
      h2 = $("<" + "input />", {
        ...
        blur: $.proxy(function (e) {
          ...
          nv = !!this.rename_node(
            obj,
            f ? $("<div></div>").text(v).text() : $("<div></div>").append($.parseHTML(v)).html()
          );
          if (!nv) {
    ...
    set_theme: function (theme_name, theme_url) {
      ...
      this.element[this.settings.core.themes.responsive ? "addClass" : "removeClass"](
        ...
        this.element
          .on(
            ...
            $.proxy(function (e, data) {
              ...
              this._data[t ? "core" : "checkbox"].selected = $.vakata.array_unique(
                this._data[t ? "core" : "checkbox"].selected
              );
            }, this)
          )
        ...
        return i &&
          (!i._disabled ||
            ($.isFunction(i._disabled) &&
              !i._disabled({ item: i, reference: vakata_context.reference, element: vakata_context.element }))) &&
          i.action
          ? i.action.call(null, {
              item: i,
              reference: vakata_context.reference,
        ...
        $.each(o, function (i, val) {
          ...
          str +=
            ...
            ($.isFunction(val._disabled) &&
              val._disabled({ item: val, reference: vakata_context.reference, element: vakata_context.element }))
          ...
          str +=
            ($.isFunction(val.label)
              ? val.label({ item: i, reference: vakata_context.reference, element: vakata_context.element })
              : val.label) +
            (val.shortcut
    ...
    $(document)
      ...
      .on("dnd_move.vakata.jstree", function (e, data) {
        ...
        if (ins && ins._data && ins._data.dnd) {
          marker.attr(
            "class",
            "jstree-" + ins.get_theme() + (ins.settings.core.themes.responsive ? " jstree-dnd-responsive" : "")
          );
          is_copy =
            data.data.origin &&
          ...
          if (e.type === "keyup" && e.which === 27) {
            ...
            data.helper
              ...
              [
                data.data.origin &&
                (data.data.origin.settings.dnd.always_copy ||
                  (data.data.origin.settings.dnd.copy && (e.metaKey || e.ctrlKey)))
                  ? "show"
                  : "hide"
              ]();
            if (lastev) {
        ...
        if (!vakata_dnd.is_drag) {
          if (
            Math.abs(e.pageX - vakata_dnd.init_x) >
              (vakata_dnd.is_touch ? $.vakata.dnd.settings.threshold_touch : $.vakata.dnd.settings.threshold) ||
